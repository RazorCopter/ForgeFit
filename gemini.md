...
- **Frontend**: (Flutter/Dart) o (Next.js + React + Tailwind CSS)
...
- **Script di Deploy:** C:\Users\gianvito.bleve\Progetti\ToolKit\deploy\deploy - forgefit.ps1 (NOTA BENE: usa SEMPRE `deploy - forgefit.ps1` e mai il file generico `deploy.ps1`, poiché il generico sovrascrive le variabili d'ambiente di Portainer azzerando le chiavi API).

## Gestione Versionamento (Bump Version)
Per aggiornare la versione dell'app (frontend e backend simultaneamente) in modo sicuro e centralizzato:
- Esegui sempre lo script Python dedicato: `python execution/bump_version.py <nuova_versione>` (es. `python execution/bump_version.py 1.9.5`).
- Lo script aggiornerà in automatico `backend/version.py`, `frontend/pubspec.yaml` (incrementando il numero di build `+X`) e `frontend/lib/core/app_version.dart`. Non aggiornare mai questi file manualmente.
